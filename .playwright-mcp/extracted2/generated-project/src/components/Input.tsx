...
export function Input({ prefix, suffix, error, style, placeholder, value, onChange }: InputProps) {
  return (
    <div style={{ position: 'relative' }}>
      {prefix && <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#999' }}>{prefix}</span>}
      <input 
        style={{ width: '100%', padding: '8px 12px', paddingLeft: prefix ? 36 : 12, paddingRight: suffix ? 36 : 12, border: error ? '1px solid #ff4d4f' : '1px solid #d9d9d9', borderRadius: 4, fontSize: 14, outline: 'none', ...style }} 
